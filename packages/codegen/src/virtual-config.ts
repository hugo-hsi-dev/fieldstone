...
import { discoverContentFiles, normalizePath } from "./collections.js";

export async function loadVirtualConfig(root: string, options: FieldstoneConfigInput) {
  const { collections, globals } = await discoverContentFiles(root);
  ...
  const storageBlock = options.storage ? `  storage: ${JSON.stringify(options.storage)},\n` : "";
