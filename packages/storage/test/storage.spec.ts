...
describe("LocalDiskStorage", () => {
  it("round-trips put/get/delete and builds urls", async () => {
    ...
    try {
      ...
      // The bytes physically land under the configured directory.
  ...
  it("rejects path-traversal keys in both the guard and the adapter", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "fs-storage-"));
    try {
...
describe("resolveStorage", () => {
  it("defaults to local disk and honors staticURL", () => {
    ...
    expect(resolveStorage({ storage: { staticURL: "/files" } }).url("x.png")).toBe(
      "/files/x.png",
    );
    expect(DEFAULT_STATIC_DIR).toBe(".fieldstone/uploads");
  });

  it("rejects unknown adapters rather than silently using local disk", () => {
